 ...
 * last modified: $LastChangedDate: 2012-04-28 01:33:46 +0300 (la, 28 huhti 2012) $ by $LastChangedBy: f4g.cnit $
 * revision:      $LastChangedRevision: 1399 $
 * 
 * short description:
 *   {To be completed}
 * ============================= /Header ==============================
	...
	public VJob getNodeTypeConstraints() {
		...
		for(ServerType s : Utils.getAllServers(model)) {
			...
			if(s.getStatus() == ServerStatusType.POWERING_OFF ||
			   s.getStatus() == ServerStatusType.POWERING_ON) {
				offlines.add(n);
			}
		}
		if(onlines.size() != 0) {
