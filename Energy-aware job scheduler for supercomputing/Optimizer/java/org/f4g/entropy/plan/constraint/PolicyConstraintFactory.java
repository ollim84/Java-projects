...
public class PolicyConstraintFactory {
		...
		if (federation != null) {
			...
			for (Policy pol : federation.getBoundedPolicies().getPolicy()) {
				...
				if (myPol.getPeriodVMThreshold() != null) {
					periodVMThreshold = pol.getIdref()
							.getPeriodVMThreshold();
					ManagedElementSet<Node> nodes = Utils
							.getNodesFromFederation(federation, src);
					if (nodes.size() != 0) {
						addPeriodVMThreshold(nodes, periodVMThreshold);
					} else {
						//if the federation contains no servers, we include them all (conservative measure for testing)
						addPeriodVMThreshold(src.getAllNodes(),
								periodVMThreshold);
					}
			...
			for (Cluster c : clusterList) {
				...
				if (c.getBoundedPolicies() != null) {
					for (Policy pol : c.getBoundedPolicies().getPolicy()) {
						PolicyType.Policy myPol = pol.getIdref();
						if (myPol.getPeriodVMThreshold() != null) {
							List<PeriodType> periodVMThreshold = pol
									.getIdref().getPeriodVMThreshold();
	...
	private void addPeriodVMThreshold(ManagedElementSet<Node> nodes, List<PeriodType> periods) {
		...
		if(model.getDatetime() != null) {
			...
			if(nodes.size() !=0 && 
			   loadType != null && loadType.getLowVMSlotsThreshold() > 0 ) {
				v.addConstraint(new SpareCPUCapacities(nodes, loadType.getLowVMSlotsThreshold()));
			}
