...
public class F4GAvgMemoryOverbooking extends F4GConstraint {
	 ...
	 */
	public F4GAvgMemoryOverbooking(ManagedElementSet<Node> ns, double m, FIT4GreenType model) {
		max = m;
		nodes = ns;
		this.model = model;
	}
	...
	public String toString() {
		return "MemoryOverbookingAvgConstraint(" + nodes.toString() + ", " + max + ")";
	}
	...
	public void inject(ReconfigurationProblem core) {
		...
		for (int i = 0; i < vms.size(); i++) {
			...
			if (isInCluster(vm.getName(), vms))
				vRAMs.add(core.createIntegerConstant("VRAMs for VM " + i,
						vm.getMemoryDemand()));
		}
		...
		IntDomainVar[] demands = vRAMs
				.toArray(new IntDomainVar[vRAMs.size()]);
		...
		for (Node n : nodes) {
			...
			eidleServer[i] = core.createEnumIntVar("NumberOfRAM" + i,
